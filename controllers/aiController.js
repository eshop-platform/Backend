...
const productToChatContext = (product) => ({
  id: product._id,
  name: product.name,
  category: product.category ? (product.category.name || String(product.category)) : "",
  price: product.price,
  rating: product.avgRating, // mapped from avgRating
  reviewCount: product.ratings ? product.ratings.length : 0, // mapped from ratings.length
...
exports.shoppingAssistantChat = async (req, res, next) => {
  try {
    ...
    // Used 'Approved' instead of 'approved' to match models/Product.js
